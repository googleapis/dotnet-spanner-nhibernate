...
namespace Google.Cloud.Spanner.NHibernate.Samples
{
    ...
    public static class SampleRunner
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (sampleMethod != null)
                {
                    ...
                    RunSampleAsync(configuration =>
                    {
                        return (Task)sampleMethod.Invoke(null, new object[] { configuration });
        ...
        private static async Task RunSampleAsync(Func<SampleConfiguration, Task> sampleMethod)
        {
        ...
        private static MethodInfo GetSampleMethod(string sampleName)
        {
            try
            {
                var sampleClass = System.Type.GetType($"{typeof(SampleRunner).Namespace}.Snippets.{sampleName}Sample");
                if (sampleClass == null)
                {
        ...
        private static async Task CreateSampleDataModel(string connectionString)
        {
            ...
            for (var i = 0; i < statements.Length; i++)
            {
                ...
                {
                    var startIndex = statements[i].IndexOf("/*", StringComparison.Ordinal);
                    var endIndex = statements[i].IndexOf("*/", startIndex, StringComparison.Ordinal) + "*/".Length;
        ...
        {
            var extraStatements = new string[length - 1];
        ...
        {
            var sampleNamespace = $"{typeof(SampleRunner).Namespace}.Snippets";
            var sampleClasses = from t in Assembly.GetExecutingAssembly().GetTypes()
                                where t.IsClass && Equals(t.Namespace, sampleNamespace) && t.Name.EndsWith("Sample")
                                select t;

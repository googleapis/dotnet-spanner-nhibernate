...
{
    /// <summary>
    /// This sample shows how to list all databases.
    /// 
    /// Run from the command line with `dotnet run ListDatabasesSample`
    /// </summary>
    public class ListDatabasesSample
    {
        public static async Task Run(SampleConfiguration configuration)
        {
            ...
            var databases =  databaseAdminClient.ListDatabasesAsync(instanceName).GetAsyncEnumerator();

            Console.WriteLine($"Databases for project `{configuration.ProjectId}` instance `{configuration.InstanceId}`:");
            
            while (await databases.MoveNextAsync())
            {
                Console.WriteLine($"\t db: `{databases.Current.DatabaseName}`");
            }
        }
    }
}
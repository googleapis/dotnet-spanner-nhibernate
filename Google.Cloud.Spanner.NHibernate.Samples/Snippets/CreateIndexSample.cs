...
namespace Google.Cloud.Spanner.NHibernate.Samples.Snippets
{
    ...
    /// <summary>
    /// This sample shows how to create an index on table.
    /// 
    ...
    public class CreateIndexSample
    {
        public static async Task Run(SampleConfiguration configuration)
        {
            ...
            //Executing DDL command to create index
            var updateDbData = await databaseAdminClient.UpdateDatabaseDdlAsync(databaseName,new []{ "CREATE INDEX Idx_Albums_ReleaseDate ON Albums(ReleaseDate)" });

...
{
    /// <summary>
    /// This sample shows how to update a database using DDL in Cloud Spanner.
    /// This example cannot be run on the Google Spanner emulator.
    /// To run this sample please fill field defaultLeader.
    /// Run from the command line with `dotnet run UpdateDatabaseSample`
    /// </summary>
    public class UpdateDatabaseSample
    {
        // TODO(developer): Replace these variables before running the sample.
        private static readonly string defaultLeader = "my-default-leader";
        ...
        public static async Task Run(SampleConfiguration configuration)
        {
            ...
            var updateDatabaseData = await databaseAdminClient.UpdateDatabaseDdlAsync(databaseName, new[]
            {
                    $"ALTER DATABASE `{configuration.DatabaseId}` SET OPTIONS (default_leader = '{defaultLeader}')"
            });

            if (updateDatabaseData.IsFaulted)
            {
                Console.WriteLine("Failed to update a database");
                throw updateDatabaseData.Exception;
            }
